...
let model, hitbox, floorbutton1, floorbutton2; 
window.onload = function(){
	...
	floorbutton1 = document.getElementById("floorButton1");
	floorbutton2 = document.getElementById("floorButton2");
	
	floorbutton1.addEventListener("click",function(){
		cam1.setAttribute("active","true");
		cam2.setAttribute("active","false");
	})
	
	floorbutton2.addEventListener("click",function(){
		cam1.setAttribute("active","true");
		cam2.setAttribute("active","false");
	})
    ...
    let x = rnd(-20,20);
    let z = rnd(-20,20);
	enemies.push(new Enemy(x,100,z));
	}
	//enemy = new Enemy(0, 0, -2)
	heal2 = new Heal(3, 2, 5, "green");
	heal3 = new Heal(5, 2, 5, "blue");
	shield = new Shield;
...
function loop(){
	...
	hitbox.setAttribute("position","0 5 1");
	model.setAttribute("src", "#520a");
	model.setAttribute("position","0 5 1");
	...
	for(let enemy of enemies){
		if(distance(enemy.enemy,cam2)<2.5 && block && !enemy.dead && shieldHealth>0){
			shieldHealth -= 0.1
		}
		if(distance(enemy.enemy,cam2)<2.5 && !block && !enemy.dead){
			enemy.attackPlayer();
		}
		if(distance(enemy.enemy,cam2)<2.5 && shieldHealth<0){
			enemy.attackPlayer();
			shield.unrotate();
			_open = false;
			block = false;
		...
		let d = distance(enemy.enemy, cam2);
		if(2.5 < d && d < 60 && !enemy.dead){
		enemy.rotateTowards(cam2); 
		enemy.forward()
		}
